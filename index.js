...
function getYears(finalsCB) {
    const yearsArr = finalsCB(fifaData).map(function(item){
        return item["Year"]
    })
    return yearsArr
};
...
function getWinners(finalsCB) {
    let winnersArr = []
    finalsCB(fifaData).forEach(function(item){
        if (item["Home Team Goals"] > item["Away Team Goals"]) {
            return winnersArr.push(item["Home Team Name"]) 
        }
        else if(item["Away Team Goals"] > item["Home Team Goals"]){
            return winnersArr.push(item["Away Team Name"])
        }
    })
    return winnersArr
};
...
function getWinnersByYear(winCB,yearsCB) {
 winCB(fifaData)
 yearsCB(fifaData)
 return `In ${yearsCB}, ${winCB} won the world cup!`
};
